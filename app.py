...
def cal_array_with_operator(array):
    """
...
def cal_operator_with_arrangement_arrays(first_array, array_3, array_result):
    remain_array = [array_3[i] for i in range(0, len(array_3))
                    if i not in range(0, len(first_array))]
    if len(first_array) > 2:
        first_new = []
        first_array = get_expression_from_arrangement_array(first_new, first_array)
    else:
        first_array = cal_array_with_operator(first_array)
    r = []
    result = []
    if len(remain_array) > 2:
        r = get_expression_from_arrangement_array(array_result, remain_array)
    else:
        if len(array_result) > 0:
            result = array_result
        r = cal_array_with_operator(remain_array)
    for op in OPERATOR:
        for e in r:
...
def get_expression_from_arrangement_array(result, array):
    """
    ...
    for i in range(0, len(array) - 1):
        ...
        result = cal_operator_with_arrangement_arrays(first_array, array, result)

    return result


def cal_expressions(result, f):
    """
...
def make_10():
    ...
    try:
        ...
        for i in list_data:
            result = []
            result = get_expression_from_arrangement_array(result, i)
            cal_expressions(result, f)
        f.close()
    except Exception as e:
        print(e)
